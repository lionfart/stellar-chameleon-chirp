...
const ShopScreen: React.FC<ShopScreenProps> = ({ items, onPurchase, onClose, playerGold }) => {
  ...
  return (
    <div className="absolute inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4"> {/* Added p-4 for mobile padding */}
      <Card className="w-11/12 max-w-md p-4 relative"> {/* Adjusted width for mobile */}
        <Button
          variant="ghost"
          size="icon"
          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div key={item.id} className="flex items-center justify-between p-3 border rounded-md bg-secondary">
                <div>
                  <h3 className="font-semibold text-lg">{t(item.id as any)}</h3>
                  <p className="text-sm text-muted-foreground">{t(`${item.id}Desc` as any)}</p>
                </div>
                <Button
